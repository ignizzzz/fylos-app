...
const CURR=[['CHF','Swiss franc'],['EUR','Euro'],['USD','US dollar'],['GBP','British pound']];
const CurrencyScreen = () => {
  const [sel,setSel]=useState('CHF');
        ...
        <div className="relative" style={{width:390,height:844,borderRadius:50,border:'8px solid #000',overflow:'hidden',backgroundColor:CREAM}}>
          ...
          <div className="absolute inset-0 overflow-y-auto" style={{background:CREAM,scrollbarWidth:'none'}}>
            <div className="pt-14 pb-5 px-5 flex items-center justify-center relative sticky top-0 z-30 pointer-events-none" style={{background:'linear-gradient(to bottom, #F7F5F2 0%, #F7F5F2 56%, rgba(247,245,242,0) 100%)'}}>
              ...
              <h1 className="text-[17px] font-bold" style={{color:INK}}>Currency</h1>
            </div>
              ...
              <Card className="mt-1">
                {CURR.map(([c,sub],i)=>(
                  <div key={c} className="relative">
                    <button onClick={()=>setSel(c)} className="w-full flex items-center gap-3 px-4 py-3.5 active:bg-black/[0.02] text-left">
                      <span className="w-9 h-9 rounded-[12px] shrink-0 flex items-center justify-center text-[11px] font-extrabold" style={{background:TINT,color:CORAL}}>{c}</span>
                      <span className="flex-1 text-[14.5px] font-semibold" style={{color:INK}}>{sub}</span>
                      <span className="w-[22px] h-[22px] rounded-full flex items-center justify-center shrink-0" style={{background:sel===c?CORAL:'transparent',border:sel===c?'none':'1.6px solid #DDD4C9'}}>{sel===c && <Check size={13} color="#fff" strokeWidth={3}/>}</span>
                    </button>
                    {i<CURR.length-1 && <div className="absolute bottom-0 left-[62px] right-0 h-px" style={{background:LINE}}/>}
                  </div>
                ))}
              </Card>
              <p className="text-[11.5px] mt-4 ml-1.5 leading-[1.5]" style={{color:TERT}}>Prices convert for display only. Providers are always paid in CHF.</p>
