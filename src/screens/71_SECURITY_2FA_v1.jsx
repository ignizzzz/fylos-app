...
import { ChevronLeft, ChevronRight, KeyRound, Smartphone, FileKey } from 'lucide-react';
        ...
        <div className="relative" style={{width:390,height:844,borderRadius:50,border:'8px solid #000',overflow:'hidden',backgroundColor:CREAM}}>
          ...
          <div className="absolute inset-0 overflow-y-auto" style={{background:CREAM,scrollbarWidth:'none'}}>
            <div className="pt-14 pb-5 px-5 flex items-center justify-center relative sticky top-0 z-30 pointer-events-none" style={{background:'linear-gradient(to bottom, #F7F5F2 0%, #F7F5F2 56%, rgba(247,245,242,0) 100%)'}}>
              ...
              <h1 className="text-[17px] font-bold" style={{color:INK}}>Two-factor authentication</h1>
            </div>
            <div className="px-4 pb-12">

              <Card className="mt-1"><Row icon={KeyRound} title="Two-factor authentication" subtitle={on?'On. A code is asked on every new device':'Off. Your password is the only lock'} trailing={<Toggle value={on} onChange={(v)=>{setOn(v);act(v?'Two-factor is on':'Two-factor is off');}}/>} last/></Card>
              <div style={{opacity:on?1:0.45,pointerEvents:on?'auto':'none',transition:'opacity 0.2s'}}>
                <SectionLabel>Method</SectionLabel>
                <Card>
                  <Row icon={Smartphone} title="Authenticator app" subtitle="Google Authenticator" rightValue="Connected" />
                  <Row icon={FileKey} title="Backup codes" subtitle="8 unused codes" onClick={()=>act('Backup codes shown once. Keep them safe')} last/>
                </Card>
              </div>
              <p className="text-[11.5px] mt-5 ml-1.5 leading-[1.5]" style={{color:TERT}}>We ask for a code when you sign in on a new device or change payout details.</p>
