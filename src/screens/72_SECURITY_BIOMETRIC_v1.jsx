...
import { ChevronLeft, ScanFace, Lock } from 'lucide-react';
        ...
        <div className="relative" style={{width:390,height:844,borderRadius:50,border:'8px solid #000',overflow:'hidden',backgroundColor:CREAM}}>
          ...
          <div className="absolute inset-0 overflow-y-auto" style={{background:CREAM,scrollbarWidth:'none'}}>
            <div className="pt-14 pb-5 px-5 flex items-center justify-center relative sticky top-0 z-30 pointer-events-none" style={{background:'linear-gradient(to bottom, #F7F5F2 0%, #F7F5F2 56%, rgba(247,245,242,0) 100%)'}}>
              ...
              <h1 className="text-[17px] font-bold" style={{color:INK}}>Biometric unlock</h1>
            </div>
            <div className="px-4 pb-12">

              <div className="flex flex-col items-center pt-6 pb-2">
                <span className="w-[88px] h-[88px] rounded-[26px] flex items-center justify-center" style={{background:TINT}}><ScanFace size={40} color={CORAL} strokeWidth={1.6}/></span>
              </div>
              <Card className="mt-4">
                <Row icon={ScanFace} title="Unlock with Face ID" subtitle="Open fylos without a passcode" trailing={<Toggle value={face} onChange={setFace}/>}/>
                <Row icon={Lock} title="Require for payments" subtitle="Confirm holds and bookings with your face" trailing={<Toggle value={pay} onChange={setPay}/>} last/>
              </Card>
              <p className="text-[11.5px] mt-4 ml-1.5 leading-[1.5]" style={{color:TERT}}>Face ID stays on this device. Your face never reaches our servers.</p>
