...
const INITIAL_NOTIFICATIONS = [
  ...
  },
  {
    ...
    actions: [
      ...
      { id: 'calendar', label: 'Add to Cal', type: 'secondary' }
    ],
    read: false,
    archived: false,
    timeGroup: 'TODAY',
...
// --- INBOX SCREEN & COMPONENTS (STEP 31) ---

const NotificationCard = ({ notification, onAction, onMarkRead, onArchive }) => {
...
const InboxScreen = ({ isOpen, onClose, notifications, setNotifications }) => {
  ...
  const handleAction = (id, actionId) => {
    // Simulate action execution & auto-mark as read
      ...
      >
        ...
        <header className="pt-14 pb-4 px-5 bg-white border-b border-black/[0.04] shrink-0 z-10">
          <div className="flex justify-between items-center w-full mb-4">
            <button 
              onClick={onClose}
              className="w-[44px] h-[44px] flex items-center justify-center bg-[#F7F7F8] rounded-full active:scale-[0.98] transition-all"
            >
...
const GenericScreen = ({ title, icon }) => (
  <ScreenContainer>
    <EmptyState icon={icon} title={title} description={`${title} profiles and management.`} />
  </ScreenContainer>
);
...
export default function App() {
  ...
  const renderScreen = () => {
    switch (displayTab) {
      ...
      case 'pets': return <GenericScreen title="Pets" icon={PawPrint} />;
      case 'services': return <GenericScreen title="Services" icon={Calendar} />;
      case 'activity': return <GenericScreen title="Activity" icon={Activity} />;
      case 'vault': return <GenericScreen title="Vault" icon={Folder} />;
