...
const _S=({options,value,onChange})=>(<div className="flex gap-2">{options.map((o)=>{const on=value===o;return(<button key={o} onClick={()=>onChange(o)} className="flex-1 h-[42px] rounded-[12px] text-[13px] font-bold active:scale-[0.97] transition-all" style={{background:on?'#FFF3EC':'#fff',color:on?CORAL:MUTED,boxShadow:on?'inset 0 0 0 1.6px '+CORAL:SHADOW}}>{o}</button>);})}</div>);
const LIBS=[['React','18.x · MIT'],['Vite','5.x · MIT'],['Tailwind CSS','3.x · MIT'],['lucide-react','ISC'],['React Router','6.x · MIT'],['Inter typeface','SIL OFL 1.1'],['Nunito typeface','SIL OFL 1.1']];
        ...
        <div className="relative" style={{width:390,height:844,borderRadius:50,border:'8px solid #000',overflow:'hidden',backgroundColor:CREAM}}>
          ...
          <div className="absolute inset-0 overflow-y-auto" style={{background:CREAM,scrollbarWidth:'none'}}>
            <div className="pt-14 pb-5 px-5 flex items-center justify-center relative sticky top-0 z-30 pointer-events-none" style={{background:'linear-gradient(to bottom, #F7F5F2 0%, #F7F5F2 56%, rgba(247,245,242,0) 100%)'}}>
              ...
              <h1 className="text-[17px] font-bold" style={{color:INK}}>Licenses</h1>
            </div>
            <div className="px-4 pb-12">

              <p className="text-[12.5px] leading-[1.5] ml-1.5 mt-1" style={{color:MUTED}}>fylos is built with open source software. Thank you to these projects.</p>
              <Card className="mt-4">
                {LIBS.map(([l,v],i)=>(
                  <div key={l} className="relative">
                    <div className="flex items-center px-4 py-3.5">
                      <span className="flex-1 text-[14px] font-semibold" style={{color:INK}}>{l}</span>
                      <span className="text-[11.5px] font-medium" style={{color:TERT}}>{v}</span>
                    </div>
                    {i<LIBS.length-1 && <div className="absolute bottom-0 left-4 right-0 h-px" style={{background:LINE}}/>}
                  </div>
                ))}
              </Card>
