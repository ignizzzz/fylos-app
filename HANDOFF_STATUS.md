...
1. ~~BATCH WIRE INTO THE FILM~~ **DONE 2026-07-16 (founder-directed edit to the locked film).** All chosen cards from section 4 are now wired into `website-live/index.html`: `#ovl-tag` is the full Layout 6 (two-line status rows, COMING SOON badge, one coral "Get {pet}'s ID" button to app.fylos.me; the QR try-block was replaced per the chosen layout), `#ovl-vet` has its 3 stages plus the **dark reception screen pinned to the desk monitor** (FYLOS · RECEPTION, incoming booking, HISTORY LOADED, dashed connector; shown at stage 2) and the Allergies consent toggle, `#ovl-groomer` is now a 3-stage swap (shop / neighbors Sofia+Marc / join with two paths + Apply to join), `#ovl-glass` P1 was already wired. Engine untouched except two data points: groomer dwell 1500→2600 and one added stage-threshold line (same pattern as vet). Verified against the approved mockups on the real film frames, full-scroll clean console.
2. **Backend for `/apply` and `/join`.** Both are front-end only right now (submit shows a thank-you state, nothing is sent). Need real submission to email or a DB. Related: the working QR product flow should give each physical tag a unique code (`/p/<id>`); the demo currently uses one shared `/found` page. See `docs/BACKEND_INTEGRATION_GUIDE.md`.
3. ~~Footer "Apply to join" links~~ **DONE 2026-07-16.** Film end now fully linked: clinic role card → `/apply` (was fylos.me), walker card already → `/join`, all 9 dead `href="#"` replaced (store badges → app.fylos.me; the socials/fine-print placeholder columns became real Explore/Partners columns linking every site page). `#finBadges` also point to app.fylos.me instead of the nonexistent `#download`.
